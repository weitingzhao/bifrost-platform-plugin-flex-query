...
from bifrost_flex_query.scheduler.enqueue import insert_job, payload_hash, trim_old_jobs
...
def test_payload_hash_deterministic() -> None:
    a = payload_hash({"as_of": "2026-08-17"})
    b = payload_hash({"as_of": "2026-08-17"})
    assert a == b
    assert len(a) == 16
    assert a != payload_hash({"as_of": "2026-08-18"})


def test_insert_job_returns_id() -> None:
    conn = _EnqueueConn()
    job_id = insert_job(conn, kind="flex-trades", payload={"as_of": "2026-08-17"}, priority=5)
    assert job_id == 1
    assert conn.committed == 1
    sql = conn.statements[0][0]
    assert "INSERT INTO ops_jobs.job_flex_ingest" in sql
...
def test_insert_job_dedup_returns_none() -> None:
    ...
    first = insert_job(conn, kind="flex-trades", payload=payload)
    second = insert_job(conn, kind="flex-trades", payload=payload)
    assert first == 1
...
def test_trim_old_jobs() -> None:
    ...
    n = trim_old_jobs(conn, keep_days=14, keep_max=2000)
    assert n == 6
    assert conn.committed == 1
    assert any("DELETE FROM ops_jobs.job_flex_ingest" in s[0] for s in conn.statements)
